...
Performance note:
- `riff start` warms a local Parakeet server in the background (when enabled), reports whether it spawned or reused an instance, and does not wait for readiness, so later `riff stop` calls are faster.
